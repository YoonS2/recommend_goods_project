/*-------------------------------------------------------  테스트데이터 */
%LET TEST_RAW=  SUSER40.AMTRAW_LINE39_A1_20210501;  /*테스트데이터 AMT */
%LET YYYYMM ='202105' ; /* EVENT월 */
%LET LINE=39;  /*생성할 중분류코드 */
%LET BIZ=A1;  /*생성할 유형*/
%LET NUM=20210501;/*예측 시점 NUM*/
...
SELECT A.BIZPL_CL_DIV_CD
			...
			/*SET_COMPOSIT_GOODS_CD; 세트를 구성하는 단품코드*/
            ,CASE WHEN E.GOODS_NM^='' THEN E.GOODS_NM ELSE D.GOODS_NM END  AS GOODS_NM
            ,A.SALE_DAY
            ,CASE WHEN C.SET_COMPOSIT_GOODS_CD^='' THEN A.QTY*C.CNVS_QTY ELSE A.QTY END AS QTY /*CNVS_QTY: 환산수량*/
			,A.AMT 
...
LEFT JOIN (SELECT *
				FROM GSSELDW.TH_MS_GOODS_COMPOSIT_CS 
				WHERE CNVS_QTY>=1)  AS C /*묶음상품 정보 */
...
HAVING (DEL1=0 AND DEL2=0)  /*판매수량 이상치이면서 매출 이상치인 것 제외*/
;
QUIT;
...
DATA SUSER40.&BIZ._&line.예측Y_RAW&NUM.;  /*TARGET용 NUM일자 자료*/
SET AMT_LINE03;
RUN;
...
LEFT JOIN (SELECT DISTINCT LINE_CD, LINE_NM, CLASS_CD, CLASS_NM, GOODS_CD, GOODS_NM
				FROM GOOD_DM) AS C 
...
IF INDEX(CLASS_NM,'온라인')>0 THEN DEL=1;
ELSE DEL=0;
IF DEL=0;
RUN;
...
RUN;
	...
	SELECT A.BIZPL_CL_DIV_CD
				...
				,C.KEY
	FROM BIZPL_TFIDF_01_&BIZ. AS A
	INNER JOIN GOODS_SEQ01 AS B 
ON A.BIZPL_CL_DIV_CD = B.BIZPL_CL_DIV_CD AND A.LINE_CD = B.LINE_CD AND A.GOODS_CD = B.GOODS_CD AND B.LINE_CD = "&LINE"
	INNER JOIN ORIGIN_SEQ01 AS C
ON A.BIZPL_CL_DIV_CD = C.BIZPL_CL_DIV_CD AND A.ORIGIN_BIZPL_CD = C.ORIGIN_BIZPL_CD 
AND A.LINE_CD = C.LINE_CD AND C.BIZPL_CL_DIV_CD = "&BIZ"
    ...
    SELECT A.*
                ,CASE WHEN 1>=A.COL1>=0.9 THEN 1 ELSE 0 END AS SEG  /*유사도가 0.9이상인 점포만 가져온다 */
...
/*	실제 판매량과 예측 판매량 간의 오차를 구하여 오차가 가장 작은 유사도를 선택 (유사도 90에서 98사이 )*/
...
/*	NOTE: Table WORK.ORIGIN_MAX created, with 2080 rows and 4 columns.*/
    ...
    SELECT "&BIZ" AS BIZPL_CL_DIV_CD
                ...
                ,CALCULATED CUM_TF_COL / CALCULATED CUM_COL1 AS PRE_TFIDF  /*예측판매량*/
    FROM COS03_&J AS A
    ...
    LEFT JOIN BIZPL_TFIDF_01_&BIZ. AS C 
ON A.BF_ORIGIN = C.ORIGIN_BIZPL_CD AND A.GOODS_CD = C.GOODS_CD AND C.BIZPL_CL_DIV_CD = "&BIZ" AND C.LINE_CD = "&LINE"
...
SELECT &J. AS SIM,SQRT(TFIDF_MSE) AS TFIDF_RMSE
, SQRT(QTY_MSE) AS QTY_RMSE /*판매량 오차*/
FROM (SELECT &J. AS SIM, MEAN(TFIDF_SSE) AS TFIDF_MSE ,MEAN(QTY_SSE) AS QTY_MSE
			FROM DIFF_&J.
			GROUP BY 1)
;
QUIT; 
DATA RMSE_&BIZ. ;  /*최종 오차 테이블*/
...
%RMSE();
	...
	SELECT A.*, B.QTY_RMSE  AS DIFF_T
, (1-B.QTY_RMSE /A.QTY_RMSE ) AS BETA /* 그래프상 오차가 꺽이는 지점 찾기 위함*/
, CASE WHEN CALCULATED BETA<0 THEN 'A' ELSE 'B' END AS DEL
	...
	SELECT A.*
	FROM DIFF_RESULT AS A
	INNER JOIN (SELECT MAX(NUM) AS NUM FROM DIFF_RESULT WHERE BETA<0) AS B ON A.NUM = B.NUM
	;
	QUIT;
	%END;

PROC SQL NOPRINT ;
SELECT COMPRESS(PUT(SIM,2.)) INTO: COS FROM DIFF_RESULT01 ;QUIT; /*최종적으로 선택된 유사도 */

%PUT ------------------------ &COS; /*로그에서 유사도 확인*/


/*최종 유사도 기준에 맞는 유사점 정의*/
PROC SQL; 
CREATE TABLE ORIGIN_&BIZ. AS
SELECT  BF_ORIGIN,COUNT(DISTINCT AF_ORIGIN) AS N
FROM COS03_&COS 
GROUP BY 1
HAVING N>=5 /*유사점이 5미만인 점포는 제외시킴*/
;
QUIT; 
PROC SQL; 
CREATE TABLE TEST_SET_&BIZ. AS
SELECT A.*
...
PROC SQL; 
CREATE TABLE SUSER40.TEST_SET_&BIZ._&line._&NUM. AS
SELECT A.*
FROM ITEM_BASED_&COS. AS A
INNER JOIN ORIGIN_&BIZ. AS B
ON A.BF_ORIGIN=B.BF_ORIGIN ;
...
DATA SUSER40.SIM_&BIZ._1 ;
SET SIM_&BIZ._1 ;
RUN;

DATA SUSER40.SIM_&BIZ._2 ;
SET SIM_&BIZ._2 ;
RUN;
PROC SQL; 
CREATE TABLE TEST_SET_&BIZ._P AS
SELECT A.*,B.유사점취급/유사점수 AS 유사점취급율 ,C.RATIO AS 유형취급율 
FROM TEST_SET_&BIZ. AS A
...
/*DATA ZTC_LIB.TEST_SET_&BIZ._P ;*/
/*SET  TEST_SET_&BIZ._P ;*/
/*RUN;*/


/**/
/*PROC SQL; */
/*CREATE TABLE SUSER40.TEST_SET_&BIZ._TEST AS*/
/*SELECT A.**/
/*FROM ITEM_BASED_&COS. AS A*/
/*INNER JOIN ORIGIN_&BIZ. AS B*/
/*ON A.BF_ORIGIN=B.BF_ORIGIN ;*/
